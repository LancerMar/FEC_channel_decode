...
void FEC_CHANNEL_DECODE::FEC_VIT_IMP::encode(char* source_data_ptr, int source_data_len, char*& encode_data_ptr, int& encode_data_len, Result& result) {
    ...
    for (int i = 0; i < source_data_len; i++) {
        ...
        state = next_state;
        encode_data.insert(encode_data.end(), output_step.begin(), output_step.end());
    }
...
void FEC_CHANNEL_DECODE::FEC_VIT_IMP::decode(double* code_data_ptr, int code_data_len, char*& decode_data_ptr, int& decode_data_len, Result& result) {
    ...
    double* euclidean_dist_set = new double[_output_reference.size()];
    double* path_matric = new double[state_count];
    for (int i = 0; i < state_count; i++) {
        path_matric[i] = 0;
    }
    double* path_matric_temp = new double[state_count];
    for (int i = 0; i < state_count; i++) {
        path_matric_temp[i] = 0;
    }
    ...
    int next_out_0;
    int next_out_1;
    double branch_matric_0;
    double branch_matric_1;

    int idx = 0;
    int input = 0;

    for (int i = 0; i < coded_data_grouping; i++) {
        // calculate the euclidean distance of coded data and all refrence
        cal_euclidean_dist_set(ptr_code_data_tmp, euclidean_dist_set);
        ...
        for (int state = 0; state < state_count; state++) {
            ...
            next_out_0 = next_out_table[pre_path_0_state][input];
            next_out_1 = next_out_table[pre_path_1_state][input];
            branch_matric_0 = path_matric[pre_path_0_state] + euclidean_dist_set[next_out_0];
            branch_matric_1 = path_matric[pre_path_1_state] + euclidean_dist_set[next_out_1];
        ...
        memcpy(path_matric, path_matric_temp, state_count * sizeof(double));
...
void FEC_CHANNEL_DECODE::FEC_VIT_IMP::gen_output_reference() {
    ...
    _output_reference_llr.resize(_output_reference.size(), std::vector<double>(_output_reference[0].size(), 0));
    // generate llr reference
    for (int i = 0; i < _output_reference.size(); i++) {
        for (int j = 0; j < _output_reference[0].size(); j++) {
            double tmp = static_cast<double>(2 * _output_reference[i][j] - 1);
            _output_reference_llr[i][j] = tmp;
        }
    }
}
...
void FEC_CHANNEL_DECODE::FEC_VIT_IMP::cal_euclidean_dist_set(double* code_data_ptr, double*& euclidean_dist_set_ptr) {
    for (int i = 0; i < _output_reference_llr.size(); i++) {
        double euclidean_dist_i = 0;
        for (int j = 0; j < _output_reference_llr[i].size(); j++) {
            euclidean_dist_i = euclidean_dist_i + code_data_ptr[j] * _output_reference_llr[i][j];
        }
        euclidean_dist_set_ptr[i] = euclidean_dist_i;
    }
}
